...
if not PAPPERS_API_KEY:
    raise EnvironmentError("ATTENTION !! Clé API Pappers manquante. Vérifie ton fichier .env.")
...
def fetch_pappers_data(siren: str) -> dict:
    ...
    try:
        ...
        print(f"ATTENTION !! Erreur API Pappers : {e}")
        return {}
...
def afficher_entreprise(data):
    ...
    if data.get('categorie_entreprise'):
        print(f"Catégorie entreprise : {data['categorie_entreprise']}")
    if data.get('capital'):
    ...
    if data.get('statut_rcs'):
        print(f"Statut RCS          : {data['statut_rcs']}")
    ...
    if siege.get("latitude") and siege.get("longitude"):
        print(f"Coordonnées GPS      : {siege['latitude']}, {siege['longitude']}")
